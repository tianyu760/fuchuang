...
function hitWords(input, words) {
  ...
  return words.filter((w) => s.includes(w));
}
...
function detectIntent(question) {
  ...
  let best = { intent: 'legal_consultation', score: 0, reason: 'default', hitKeywords: [] };
  Object.keys(INTENT_KEYWORDS).forEach((intent) => {
    const hits = hitWords(q, INTENT_KEYWORDS[intent]);
    if (hits.length > best.score) {
      best = { intent, score: hits.length, reason: 'keyword_match', hitKeywords: hits };
    }
  });
...
function isKeywordConflict(question, historyText) {
  ...
  if (!q || !h) return false;
  const qCrime = hitWords(q, CRIME_DOMAIN).length > 0;
  const qLoan = hitWords(q, LOAN_DOMAIN).length > 0;
  const hCrime = hitWords(h, CRIME_DOMAIN).length > 0;
  const hLoan = hitWords(h, LOAN_DOMAIN).length > 0;
  return (qCrime && hLoan) || (qLoan && hCrime);
}

function sanitizeHistoryByIntent(history, question) {
  const rows = Array.isArray(history) ? history.slice(-12) : [];
  if (!rows.length) return [];
  const historyText = rows.map((m) => String((m && m.content) || '')).join('\n');
  if (isKeywordConflict(question, historyText)) return [];
  return rows.filter((m) => m && (m.role === 'user' || m.role === 'assistant')).map((m) => ({
    role: m.role,
